...
### seq 102 · 2026-09-20T07:25:55Z · audit · `record:d8-deterministic-reduction`

stage=0_hypothesis · verdict=open · counts_as_trial=False

_Metrics_: `columns_regenerated`=5, `deviation_ulp_vs_wmps_np_dot`={'hma_21': 4.0, 'hma_55': 4.0, 'wma_20': 3.0, 'wma_55': 4.0, 'wma_9': 2.0}, `reduction_after`=math.fsum (correctly rounded), `reduction_before`=np.dot (BLAS-dispatched), `same_machine_regeneration_drift_ulp`=5.0, `tests_passed_with_wmps`=523, `ulp_bound_retired`=True, `wma_9_was_exact_on_regeneration`=True, `wmps_changed`=False, `x22_exact_columns_after`=8, `x22_exact_columns_before`=3

> The five np.dot columns of the D8 golden fixture (wma_9/20/55, hma_21/55) were bounded at 16 ULP rather than asserted exact, on the reading that BLAS kernel selection is a property of the CPU. Regenerating to close that turned up something larger. The generator was re-run on the SAME machine that produced the original fixture, against WMPS sources whose three sha256 all still match the manifest and input whose sha256 matches: wma_20, wma_55, hma_21 and hma_55 came back up to 5 ULP different, and wma_9 — the column seq=98 recorded as the one CI could not reproduce — came back EXACT. numpy had moved under the fixture. So the drift is not only across CPUs, it is across time on one CPU, and the set of drifting columns is itself unstable. A reference that does not reproduce on its own machine pins a toolchain, not arithmetic, and no regeneration of the np.dot values could have converged. FIX: wma now reduces with math.fsum, which is correctly rounded, so the value is fixed by IEEE-754 rather than by a kernel, a thread count or a numpy release. Applied to both copies (resources and research.engines) so T11's bit-for-bit assertion between them still holds. The five columns were regenerated from that implementation and now reproduce EXACTLY; X22 asserts exact equality on all eight columns as the spec wrote it, and its ULP bound is retired. DELIBERATE DEVIATION: the port now sits 2-4 ULP from WMPS's np.dot and is the more accurate of the two (fsum returns the nearest float64 to the exact weighted sum). WMPS is unchanged, being frozen under spec 1.3, so the live app runs arithmetic a few ULP less accurate than the backtester. The manifest records the measured per-column deviation, and the port-vs-WMPS test is bounded rather than exact for those five. Acceptance criterion 8 is met by exact reproduction now, rather than by a logged deviation. INCIDENTAL: pandas read_csv's default float parser is lossy at the last bits; only float_precision='round_trip' (or Python float() on the string, which the X22 helpers already use) reads these fixtures back exactly. A verification harness written without it reports ~2 ULP of drift that is not in the file. Not a trial: this is numerical provenance, not a hypothesis test.

_hash_: `9f95e7c2608d1a77…` · _prev_: `cb5cc076f8c2fe20…`

### seq 103 · 2026-09-20T07:42:36Z · parity_fixture · `record:t9a-flood_tide_h1-mask-on`

stage=0_hypothesis · verdict=open · counts_as_trial=False

_Metrics_: `additions_channel`={'100': 0.0079, '20': 0.0066, '50': 0.0081}, `e_ratio_off`={'100': 1.1835, '20': 1.1349, '50': 1.1294}, `e_ratio_on`={'100': 1.1308, '20': 1.1444, '50': 1.1432}, `flags_reported`=['weekend_gap'], `gained_attributed_cooldown`=23, `lost_attributed_cooldown`=8, `lost_attributed_weekend_gap`=718, `mode`=mask_on, `n_gained`=23, `n_lost`=726, `n_retained`=943, `n_signals_off`=1669, `n_signals_on`=966, `normaliser_channel`={'100': 0.0193, '20': 0.0291, '50': 0.0307}, `removals_channel`={'100': -0.08, '20': -0.0262, '50': -0.025}, `rollover_annihilates_windows_over_bars`=24, `rollover_signal_count`=0, `unattributed`=0

> Acceptance criterion 3, T9a half. The seq=31 flood_tide_h1 entry was re-run with the tradability mask on and the divergence decomposed into the two channels the spec requires, measured additively: E(S_off,ATR_off) -> E(S_retained,ATR_off) -> E(S_retained,ATR_on) -> E(S_on,ATR_on), so removals, normaliser and additions sum exactly to the total. The normaliser channel is measured on the intersection of retained signals under both ATRs, isolating it from the signal-set change exactly as REWRITE.md T9a specifies. WEEKEND_GAP configuration: 1669 signals -> 966 (943 retained, 726 lost, 23 gained). E-Ratio h=20 1.1349 -> 1.1444, h=50 1.1294 -> 1.1432, h=100 1.1835 -> 1.1308. The normaliser channel is POSITIVE at every horizon (+0.029/+0.031/+0.019) while removals are negative, so the mask makes the surviving signals look better by shrinking the ATR it divides by, and simultaneously removes signals that were carrying the measured edge. Reporting one net number would have hidden both. ATTRIBUTION: every changed signal is traced. 718 of 726 losses go directly to weekend_gap. The remaining 8 losses and all 23 gains are traced transitively through the re-entry cooldown, which couples signals to each other in BOTH directions — a removed signal un-suppresses a later one, an added one suppresses a later one — so attribution runs to a fixed point rather than in one pass. The cooldown cause is reported separately from the mask flags because it reaches signals no flag goes near. Unattributed: 0. STRUCTURAL FINDING (rollover): adding ROLLOVER_WINDOW takes the signal count to ZERO. The flag fires once per day, so under R3's WINDOW rule the longest uncontaminated run is ~23 H1 bars and a 55-bar Donchian can essentially never be computed. This is an interaction between the rule and the flag's period, not a property of the data: ANY window indicator whose lookback exceeds the shortest recurring flag's period is annihilated the same way. It is pinned by a test. The weekend-only configuration is therefore the one the channels are reported under, and the rollover result is reported as the finding it is rather than buried as a degenerate run. Not a trial: this measures a known, intended behaviour change. The flood_tide_h1 verdict (SHELVED at seq=34) is not reopened.

_hash_: `a939aefe5180d569…` · _prev_: `9f95e7c2608d1a77…`

### seq 104 · 2026-09-20T07:49:20Z · parity_fixture · `record:t9b-crest_n_keel-mask-on`

stage=0_hypothesis · verdict=open · counts_as_trial=False

_Metrics_: `H1_atr_accumulator_shift`=956, `H1_clean_run_median`=114, `H1_diverged`=866, `H1_gained`=39, `H1_lost`=1089, `H1_max_lookback`=61, `H1_n_trades_off`=1973, `H1_n_trades_on`=923, `H1_window_contamination`=1034, `H4_atr_accumulator_shift`=718, `H4_clean_run_median`=29, `H4_diverged`=609, `H4_gained`=30, `H4_lost`=960, `H4_max_lookback`=16, `H4_n_trades_off`=1595, `H4_n_trades_on`=665, `H4_window_contamination`=878, `annihilated_timeframes`=[], `arm`=fixed, `criterion_3_complete`=True, `engine_modified`=False, `flags_applied`=['weekend_gap'], `fully_attributed`=True, `mode`=mask_on, `tests_passed_ci_rehearsal`=525, `unattributed`=0

> Acceptance criterion 3, T9b half. The seq=49 walk-forward's selection-independent 'fixed' arm was re-run with the weekend mask on and every differing trade traced to a named cause. The 'selected' arm is excluded deliberately: re-running its per-fold training search under the mask would confound different parameters with different indicator values in one comparison. cnk_engine is NOT modified — it is the frozen generator behind the mask-off fixture, so the masked indicator arrays are computed outside it and passed in, and both runs call the identical engine function. RESULTS. H1: 1973 -> 923 trades (1089 lost, 39 gained, 866 diverged). H4: 1595 -> 665 (960 lost, 30 gained, 609 diverged). Unattributed: 0 at both. THE DIVERGED CATEGORY IS THE FINDING. 866 H1 and 609 H4 trades kept their entry bar and moved their exit. A trade-count comparison shows none of them, and the first attribution pass missed almost all of them too, because it looked for a masked bar inside the entry's lookback. Their cause is not local: the Wilder ATR is an ACCUMULATOR, so running it on the compacted series makes EVERY value after the first masked bar differ, and that moves the stop distance, the size, and the trailing exit for trades hundreds of bars away. Attribution is therefore split into two named mechanisms — window contamination (local, bounded by the lookback) and ATR accumulator shift (unbounded) — and measured against the indicator arrays actually fed to the engine rather than inferred from the bar index. A diverged trade is checked across its whole holding period, since a trailing stop reads every bar. STARVATION MARGIN. Clean runs are one trading week: ~114 bars at H1, ~29 at H4. The fixed arm's longest lookback is 61 at H1 and 16 at H4, so nothing is annihilated here — but that is the luck of one frozen config, not a property of the strategy. A 55-bar HMA is comfortable at H1 and impossible at H4. Both numbers are reported per timeframe on every run and a test fails if any timeframe goes empty. With seq=103 this completes acceptance criterion 3. Not a trial: the crest_n_keel verdict is not reopened.

_hash_: `c4c957ed0505b516…` · _prev_: `a939aefe5180d569…`

### seq 105 · 2026-09-20T12:51:23Z · audit · `record:commission-measured-zero`

stage=0_hypothesis · verdict=open · counts_as_trial=False

_Metrics_: `constant_changed`=False, `constant_usd_per_lot_rt`=7.0, `gold_dispute_status`=MOOT (Razor-only question), `implies_account_type`=Standard, `largest_lot`=0.03, `measured_commission_usd_per_lot_rt`=0.0, `n_symbols`=5, `n_trade_deals`=358, `non_zero_commission_deals`=0, `non_zero_swap_deals`=24, `provenance_after`=MEASURED, `provenance_before`=BROKER_PUBLISHED, `server`=PepperstoneKE-MT5-Live01, `slip_still_unmeasured`=True, `span`=2025-02-10..2026-04-13, `tests_passed`=548, `xauusd_measured_spread_usd_per_oz`=0.17, `xauusd_model_spread_usd_per_oz`=0.22
...
### seq 107 · 2026-09-20T18:27:59Z · audit · `record:gate-power-curve`

stage=0_hypothesis · verdict=open · counts_as_trial=False

_Metrics_: `binding_gate_low_sr`=dsr, `conclusion`=information, not breadth, `detection_floor_annualised`=1.2, `false_positive_rate`=0.0, `inert_gates`=['n_trades'], `mintrl_floor_at_114`=1.66, `n_obs_per_fold_rejected_as_unit`=114, `n_obs_pooled`=2000, `n_trials_dsr`=26, `observed_mechanism_oos_sharpe_range`=0.27-0.41, `pass_rate_by_sr`={'0.0': 0.0, '0.3': 0.0, '0.5': 0.0, '0.8': 0.15, '1.2': 0.75, '1.5': 0.7}, `r8_fires`=True, `tightening_cost_in_detection`=0.0, `trials_per_point`=20
