"""Orchestrator: run signal_edge over avwap_multibar_reclaim_m15.

DIAGNOSTIC reference measurement. No edge-magnitude verdict is rendered.
"""
...
def run_window(signal: pd.Series, ohlc: pd.DataFrame, window: int,
    ...
    out = {
        ...
        "e_ratio_long": _er(ps_w[ps_w["direction"] > 0]),
        "e_ratio_short": _er(ps_w[ps_w["direction"] < 0]),
    ...
    if triggers is not None and window == PRIMARY_WINDOW:
        sb = triggers["sweep_bars"]
        ps_w = ps_w.copy()
        ps_w["sweep_bars"] = sb.reindex(ps_w["signal_time"]).to_numpy()
        for k in (1, 2, 3):
            out[f"e_ratio_sweep_{k}bar"] = _er(ps_w[ps_w["sweep_bars"] == k])
            out[f"n_sweep_{k}bar"] = int((ps_w["sweep_bars"] == k).sum())
    return out
...
def main() -> Path:
    ...
    n_sb = {k: int(((triggers["signal"]) & (triggers["sweep_bars"] == k)).sum())
            for k in (1, 2, 3)}
    print(f"  triggers: total={n_total} long={n_long} short={n_short}")
    print(f"  by sweep_bars: {n_sb}")
    ...
    artifact = {
        "hypothesis_id": "avwap_multibar_reclaim_m15",
        ...
        "trigger_count_sweep_1bar": n_sb[1],
        "trigger_count_sweep_2bar": n_sb[2],
        "trigger_count_sweep_3bar": n_sb[3],
    ...
    out_path = ARTIFACTS_DIR / f"avwap_multibar_reclaim_signal_edge_{ts}.json"
